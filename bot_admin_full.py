...
# Standalone central admin bot runtime profile (no feature flags needed).
try:
...
_legacy_streamflow_token = (os.getenv("BOT_TOKEN") or "").strip()
if not (os.getenv("STREAMFLOW_BOT_TOKEN") or "").strip() and _legacy_streamflow_token:
    os.environ["STREAMFLOW_BOT_TOKEN"] = _legacy_streamflow_token

_admin_center_token = (os.getenv("ADMIN_CENTER_BOT_TOKEN") or "").strip()
if not _admin_center_token:
    raise RuntimeError("ADMIN_CENTER_BOT_TOKEN is required for bot_admin_full.py")
os.environ["BOT_TOKEN"] = _admin_center_token
